...
class Attacker(ch.nn.Module):
    ...
    def forward(self, x, target, *_, constraint, eps, step_size, iterations,
        ...
        if orig_input is None: orig_input = x.detach()  # x is cpu
        orig_input = orig_input.cuda()  # orig_input is gpu
        ...
        def calc_loss(inp, target):
            ...
            '''
            if should_normalize:
                inp = self.normalize(inp)  # inp is cpu
            output = self.model(inp)  # inp is gpu, output is gpu
            if custom_loss:
                return custom_loss(self.model, inp, target.cuda())  # target is cpu
