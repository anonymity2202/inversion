...
class DataSet(object):
    ...
    def override_args(self, default_args, new_args):
        ...
        if len(extra_args) > 0: raise ValueError("Invalid arguments: {extra_args}")
        for k in kwargs:
            ...
            if no_nones and (not isinstance(kwargs[k], req_type)):
                raise ValueError("Argument {k} should have type {req_type}")
    ...
    * Summer <-> Winter
    ...
    def __init__(self, data_path, **kwargs):
        ...
        assert ds_name in valid_names, \
                "path must end in one of {valid_names}, not {ds_name}"
        ds_kwargs = {
            'num_classes': 2,
